...
const extendedCollectionsApi = rootApi.injectEndpoints({
  endpoints: (build) => ({
    ...
    getCollectionsByUserId: build.query<ICollectionForIUser[], idType>({
      ...
      transformResponse: (response: IResponse<{ collections: ICollectionForIUser[] }>) =>
    ...
    getAllowedToViewCollections: build.query<ICollectionForIUser[], idType>({
      ...
      transformResponse: (
        response: IResponse<{ allowedToViewCollections: ICollectionForIUser[] }>
      ) => response.data.allowedToViewCollections
    }),
    getCurrentUserCollections: build.query<ICollection[], void>({
      query: () => ({
        url: `/collections/current`
      }),
      transformResponse: (response: IResponse<{ collections: ICollection[] }>) =>
        response.data.collections,
      providesTags: ['Collection']
    }),
    createCollection: build.mutation<
      IResponseWithMessage<{ collection: ICollection }>,
      {
        body: {
          tags: string[]
          title: string
    ...
    addAuthorToCollection: build.mutation<
      unknown,
      { collectionId: string; authorId: string; role: 'ADMIN' | 'AUTHOR' }
    >({
    ...
    changeAuthorRoleInCollection: build.mutation<
      unknown,
      { collectionId: string; authorId: string; role: 'ADMIN' | 'AUTHOR' }
    >({
      query: ({ collectionId, authorId, role }) => ({
    ...
    changeCollectionInfo: build.mutation<
      unknown,
      { collectionId: string; title: string; tags: string[] }
    >({
    ...
    deleteRequestedUserFromCollection: build.mutation<
      unknown,
