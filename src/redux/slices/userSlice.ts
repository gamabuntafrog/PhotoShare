...
export interface IResponseWithMessage<T> {
  code: number
  data: T
  status: string
  message: string
...
export const getCurrentUser = createAsyncThunk<
  { user: ICurrentUser | null; token: string | null },
  void,
  { state: RootState }
>('users/getCurrent', async (args, { getState, rejectWithValue, dispatch }) => {
...
export const login = createAsyncThunk<
  { user: ICurrentUser; token: string },
  { email: string; password: string },
  { state: RootState }
>('users/getCurrent', async (args, { getState, rejectWithValue }) => {
  const { userReducer } = getState()
...
>('users/login', async ({ username, email, password }) => {
  const response: { status: string; code: number; data: { user: ICurrentUser; token: string } } =
    await fetch(`${authBaseURL}/register`, {
...
export const { disableLoading, subscribeToUser, unsubscribeFromUser, changeLanguage } =
  userSlice.actions
