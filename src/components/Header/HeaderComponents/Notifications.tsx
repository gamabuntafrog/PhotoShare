...
const longPollingInterval =
  process.env.NODE_ENV || process.env.NODE_ENV === 'development' ? 60000 : 15000
...
function Notification({ notification, handleClose }: INotificationProps) {
  ...
  if (type === 'subscribe') {
    return (
      ...
      >
        ...
        <ListItemText>{t('subscribed', { username })}</ListItemText>
      </StyledCustomMenuItem>
    )
  }
  ...
  if (type === 'unsubscribe') {
    return (
      ...
      >
        ...
        <ListItemText>{t('unsubscribed', { username })}</ListItemText>
      </StyledCustomMenuItem>
    )
  }
  if (type === 'likePost' || type === 'unlikePost') {
    ...
    const message =
      type === 'likePost' ? t('likedPost', { username }) : t('unlikedPost', { username })
  ...
  if (type === 'savePost') {
    ...
    return (
      ...
      >
        ...
        <ListItemText>{t('savedPost', { username })}</ListItemText>
      </StyledCustomMenuItem>
    )
  }
  ...
  if (type === 'addUserToCollection' || type === 'deleteUserFromCollection') {
    ...
    const message =
      type === 'addUserToCollection'
        ? t('addedUserToCollection', {
            username,
            collectionTitle
          })
  ...
  if (type === 'acceptJoinToCollectionRequest' || type === 'declineJoinToCollectionRequest') {
    ...
    const message =
      type === 'acceptJoinToCollectionRequest'
        ? t('acceptedJoinToCollectionRequest', {
            username,
            collectionTitle
          })
  ...
  if (type === 'addCommentToPost') {
    ...
    return (
      ...
      >
        ...
        <ListItemText>{t('addCommentToPost', { username, text: comment.text })}</ListItemText>
      </StyledCustomMenuItem>
    )
  }
  ...
  if (type === 'addReplyToComment') {
    ...
    return (
      ...
      >
        ...
        <ListItemText>{t('addReplyToComment', { username, text: comment.text })}</ListItemText>
      </StyledCustomMenuItem>
    )
  }
...
export default function Notifications() {
  ...
  const {
    ...
    isLoading,
    isError
  } = extendedNotificationsApi.useGetNotificationsQuery(undefined, {
    ...
    <Box>
      ...
      >
        {(isError || notifications.length === 0) && (
          <StyledCustomMenuItem onClick={handleClose}>
            <ListItemText>List is clear</ListItemText>
          </StyledCustomMenuItem>
        )}
        {isLoading ? (
          ...
          notifications.map((notification, index) => (
            <Notification key={index} notification={notification} handleClose={handleClose} />
          ))
        )}
      </StyledCustomMenu>
    </Box>
