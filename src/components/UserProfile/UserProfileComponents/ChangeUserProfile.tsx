...
export default function ChangeUserProfile({
  ...
  return (
    <FormGroup sx={styles.formWrapper}>
      <Button type="button" sx={{ my: 1 }}>
        <InputLabel sx={styles.inputLabel} htmlFor="avatar">
          {t('changeAvatarButtonAsLabel')}
        </InputLabel>
        <input {...register('imageList')} id="avatar" type="file" accept="image/*" hidden />
      </Button>

      <InputLabel error={!!usernameError} htmlFor="username">
        {usernameLabel}
      </InputLabel>
      <OutlinedInput
        ...
        sx={{ my: 1 }}
      />
      <Button type="button" color="error" sx={{ mb: 1 }} onClick={turnOffChangingMode}>
        {t('cancelChangesButton')}
      </Button>
      <Button disabled={isErrors} type="submit" onClick={onSubmit}>
        {t('saveChangesButton')}
      </Button>
    </FormGroup>
  )
