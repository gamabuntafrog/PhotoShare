...
export default function UserProfile() {
  ...
  const { isSubscribed, toggleSubscribe, subscribersCount } = useToggleSubscribe(
    id,
  ...
  return (
    <Box>
      ...
      <Box sx={styles.wrapper}>
        ...
        <Box sx={styles.userInfoWrapper}>
            ...
            <>
              ...
              <Box sx={styles.userInfoSecondWrapper}>
                ...
                <Typography sx={styles.infoItem}>
                  {' '}
                  {t('subscribesCount', { subscribesCount })}
                </Typography>
                <Typography sx={styles.infoItem}>
                  {t('subscribersCount', { subscribersCount })}
                </Typography>
              </Box>
          ...
          {isProfileOfCurrentUser && !isChangingMode && (
            <Button onClick={turnOnChangingMode}>{t('changeProfileButton')}</Button>
          )}
          {!isProfileOfCurrentUser && (
            <Button variant="outlined" sx={{ mt: 1 }} onClick={onToggleSubscribe}>
      ...
      <Box sx={styles.navLinksWrapper}>
        <StyledHeaderNavLink className="first" to={``} end>
          {t('linkToPosts')}
        </StyledHeaderNavLink>
      ...
      <Box sx={{minHeight: '50vh'}}>
        <React.Suspense fallback={<MiniLoader center />}>
          <Outlet />
        </React.Suspense>
      </Box>
    </Box>
