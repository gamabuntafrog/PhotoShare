...
export default function PostItemBottomButtonsWrapper({
  onToggleLike,
  isLiked,
  likesCount
}: IPostItemLikesContainerProps) {
  const styles = useSx(postItemStyles)

  return (
    <Box sx={styles.postBottomButtonsWrapper} className="buttonsBar">
      <IconButton sx={styles.likeButton} onClick={onToggleLike}>
        {isLiked ? <FavoriteIcon color="secondary" /> : <FavoriteBorderIcon />}
      </IconButton>
      <Typography sx={{ ml: 0.5, color: 'currentColor' }}>{likesCount}</Typography>
    </Box>
  )
}
