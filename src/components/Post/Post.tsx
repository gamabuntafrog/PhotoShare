...
export default function Post() {
  ...
  const currentUserId = useAppSelector(
    (state) => (state.userReducer as IUserSliceAuthorized).user._id
  )
  ...
  const [post, { toggleLike, toggleSave, updateSavesInfo, deletePost }] = usePostActions({
    initPost: data
  })
  ...
  if (!post || !post.author) {
    return (
      <>
        ...
        <Container sx={{ ...styles.postContainer, height: '90vh', display: 'flex', alignItems: 'center' }}>
  ...
  return (
    <>
      <Container sx={styles.postContainer}>
        ...
        />
        <Box key={postId} sx={styles.postWrapper}>
          <StyledPostImage src={postImageURL} />
