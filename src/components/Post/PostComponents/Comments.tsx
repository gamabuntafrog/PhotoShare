...
export default function Comments({ postId, comments: initComments }: ICommentsProps) {
  const currentUserAvatarURL = useAppSelector(
    (state) => (state.userReducer as IUserSliceAuthorized).user.avatar.url
  ) as string
  ...
  return (
    <Box sx={styles.container}>
      ...
      <Box sx={styles.writeCommentContainer(commentType === commentsType.reply)}>
        <Avatar src={currentUserAvatarURL} sx={{ mr: 1 }} />
        <Box sx={styles.inputWrapper}>
          {commentType === commentsType.reply && (
            <Box sx={styles.replyContainer} onClick={chooseComment}>
