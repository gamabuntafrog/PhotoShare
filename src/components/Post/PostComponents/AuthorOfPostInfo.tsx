...
export default function AuthorOfPostInfo({
  ...
  const { toggleSubscribe, isSubscribed, subscribersCount } = useToggleSubscribe(
    authorId,
  ...
  return (
    <Box sx={styles.container}>
      <NavLink style={styles.wrapper} to={`/users/${authorId}`}>
      ...
      {!isUserAuthorOfPost && (
        <Button sx={styles.subscribeButton} variant="contained" onClick={onToggleSubscribe}>
          {userActionsButton}
        </Button>
      )}
    </Box>
