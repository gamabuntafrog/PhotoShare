...
export default function AddUserToCollection({ collectionId }: IAddAuthorToCollectionModalProps) {
  ...
  const showNotFoundCondition =
    (users && users.length === 0) || (!users && debouncedQuery.length > 2)

  const onChangeUsernameQuery = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) =>
  ...
  return (
    <>
      ...
      {showNotFoundCondition && (
        <Typography variant="h5" sx={styles.notFound}>
          {t('notFoundMessage')}
        </Typography>
      )}
    </>
