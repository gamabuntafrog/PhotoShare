...
function UserItem({ user, collectionId }: IUserItemProps) {
  ...
  const [deleteRequestedUser] =
    extendedCollectionsApi.useDeleteRequestedUserFromCollectionMutation()
  ...
  return (
    <ListItem sx={styles.userItem} key={authorId}>
      <ListItemAvatar sx={styles.avatarWrapper}>
        <NavLink to={`/users/${authorId}`}>
          <Avatar sx={styles.avatar} src={avatar || ''} />
        </NavLink>
      </ListItemAvatar>
      <ListItemText sx={styles.usernameWrapper}>
        <NavLink to={`/users/${authorId}`}>{username}</NavLink>
      </ListItemText>
      <Button
        variant="contained"
        id="basic-button"
      ...
      <StyledCustomMenu anchorEl={anchorEl} open={isAnchorEl} onClose={handleClose}>
        ...
        >
          <ListItemText sx={{ mr: 1 }}>{t('asAuthor')}</ListItemText>
        </StyledCustomMenuItem>
        <StyledCustomMenuItem
          onClick={() => {
            handleClose()
        ...
        >
          <ListItemText sx={{ mr: 1 }}>{t('asAdmin')}</ListItemText>
        </StyledCustomMenuItem>
        <StyledCustomMenuItem
          onClick={() => {
            handleClose()
        ...
        >
          <ListItemText sx={{ mr: 1 }}>{t('asViewer')}</ListItemText>
        </StyledCustomMenuItem>
      </StyledCustomMenu>
    </ListItem>
  )
...
export default function UsersList({ users, collectionId }: IUsersListProps) {
  return (
    <List>
      {users?.map((user) => (
        <UserItem user={user} collectionId={collectionId} key={user._id} />
      ))}
    </List>
  )
}
