...
import useGetPostsByCollectionIdWithInfiniteScroll from '../../redux/api/hooks/useGetPostsByCollectionIdWithInfiniteScroll'
          ...
          <Box sx={styles.buttonsWrapper}>
            ...
            {!isAuthor && !isAdmin && !isViewer && !isInQueue && (
              <Button
                sx={{ mx: 1 }}
                variant="contained"
                onClick={() => sendRequest({ collectionId: id })}
              >
                {t('sendRequest')}
              </Button>
            )}
            {isInQueue && (
              <Button
                sx={{ mx: 1 }}
                variant="contained"
                onClick={() => unsendRequest({ collectionId: id })}
              >
                {t('unsendRequest')}
              </Button>
            )}
